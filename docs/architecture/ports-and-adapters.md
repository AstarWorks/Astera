...
| [`IScheduler`](../../plugin/application/src/main/kotlin/com/astarworks/astera/application/port/outbound/IScheduler.kt) — currentTick + runLater(Duration) + runRepeating + runAsync + `suspend awaitTicks` + main/async CoroutineDispatcher (ADR-0011/0012 follow-up) | `PaperScheduler` with BukkitMainDispatcher / BukkitAsyncDispatcher | `FoliaScheduler` (region-based, Phase 2 mid+) — port `IRegionScheduler` 用宣言は将来 |
